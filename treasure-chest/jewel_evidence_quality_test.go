...
func TestScanConflictingJewels_FlagsOverlappingRefsWithDifferingStatement(t *testing.T) {
	t.Parallel()
	jewels := []Jewel{
		{ID: "jewel-1", SourceRefs: []string{"chest#a"}, Statement: "X is true", Status: chestdomain.JewelStatusAccepted},
		{ID: "jewel-2", SourceRefs: []string{"chest#a"}, Statement: "X is false", Status: chestdomain.JewelStatusAccepted},
	}
...
func TestScanConflictingJewels_FlagsOverlappingRefsWithDifferingStatus(t *testing.T) {
	...
	jewels := []Jewel{
		{ID: "jewel-1", SourceRefs: []string{"chest#a"}, Statement: "X is true", Status: chestdomain.JewelStatusAccepted},
		{ID: "jewel-2", SourceRefs: []string{"chest#a"}, Statement: "X is true", Status: chestdomain.JewelStatusDeprecated},
	}
	found := ScanConflictingJewels("chest-a", jewels)
	require.Len(t, found, 1)
...
func TestScanConflictingJewels_OverlapWithIdenticalContentNeverFlagged(t *testing.T) {
	...
	jewels := []Jewel{
		{ID: "jewel-1", SourceRefs: []string{"chest#a"}, Statement: "X is true", Status: chestdomain.JewelStatusAccepted},
		{ID: "jewel-2", SourceRefs: []string{"chest#a"}, Statement: "X is true", Status: chestdomain.JewelStatusAccepted},
	}
	assert.Empty(t, ScanConflictingJewels("chest-a", jewels))
}
