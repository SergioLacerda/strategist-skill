...
func TestJewelSpec_NoPublicMultiCommandMiningWorkflow(t *testing.T) {
	...
	scanSource := readFile(t, filepath.Join(repoRoot(t), "treasure-chest", "cli", "treasure_chest_scan.go"))
	if !strings.Contains(scanSource, "Hidden: true") {
		t.Error("treasure-chest/cli/treasure_chest_scan.go: scan command must be Hidden (internal phase, not public UX)")
	}
	for _, stale := range []string{"treasure_chest_mine.go", "treasure_chest_jewel.go"} {
		if _, err := os.Stat(filepath.Join(repoRoot(t), "treasure-chest", "cli", stale)); err == nil {
			t.Errorf("treasure-chest/cli/%s must not exist — removed/renamed into treasure_chest_items.go", stale)
		}
	}
}
...
func TestJewelSpec_NewJewelStatuses(t *testing.T) {
	...
	statusFn := readFile(t, filepath.Join(repoRoot(t), "treasure-chest", "domain", "jewel_grade.go"))
	for _, status := range []string{`"proposed"`, `"accepted"`, `"verified"`, `"deprecated"`} {
		if !strings.Contains(statusFn, status) {
			t.Errorf("treasure-chest/domain/jewel_grade.go: missing jewel status constant %s", status)
		}
	}
}
...
func TestJewelSpec_LegacyActiveRejectedAfterMigration(t *testing.T) {
	...
	if !strings.Contains(statusFn, "jewelStatusLegacyActive") {
		t.Error("treasure-chest/domain/jewel_grade.go: must explicitly recognize and reject the legacy \"active\" status")
	}
	if !strings.Contains(statusFn, "migrate-status") {
		t.Error("treasure-chest/domain/jewel_grade.go: legacy active rejection must point at the migrate-status command")
	}

	itemsSource := readFile(t, filepath.Join(repoRoot(t), "treasure-chest", "cli", "treasure_chest_items.go"))
	if !strings.Contains(itemsSource, "migrate-status") {
		t.Error("treasure-chest/cli/treasure_chest_items.go: must expose a migrate-status subcommand for the one-time active -> accepted migration")
	}
