---
title: internal/eval Phase 2 targets fixture-based content assertions, not a provider-invocation interface
date: 2026-08-04
status: accepted
mission_id: 20260804-eval-fake-provider
---

# ADR — internal/eval Phase 2: Fixture-Based Content Assertions, Not FakeProvider
...
E/F/G, B1–B3, D1/D4) requiring scripted LLM-response content. The prior
ADR (`20260804-test-framework-v2-adr.md`, DEC-1) established that no such
interface currently exists in `internal/domain` and routed Phase 1 around
it, but left open whether one *should* be built to unblock Phase 2.
...
- **Add a minimal `internal/domain` interface purely for test purposes**
  (the option `20260804-test-framework-v2-adr.md` already rejected for
  Phase 1). Rejected again, on stronger grounds: this mission's evidence
  shows no present or foreseeable in-process caller exists for such an
  interface in this architecture — it would be dead abstraction from day
  one, not merely premature.
...
- `SQ-002` (mission-harvesting) becomes a explicit prerequisite for the
...
- Future missions extending `internal/eval` should re-read this ADR and
  `20260804-test-framework-v2-adr.md` together before re-litigating either
  decision, and should check whether Strategist's own execution model has
  changed (which would be the only thing that could revive the original
  `FakeProvider` premise).
