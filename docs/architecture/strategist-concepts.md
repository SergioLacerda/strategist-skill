...
Abilities are internal routines that run inside a Role/phase. Unlike Weapons, they are not configurable, not swappable, and have no `active.yaml` entry — they are built into Strategist itself (see `skill.yaml#taxonomy`). There are five:

| Ability | Runs in | What it does |
|---------|---------|--------------|
| **LEVELING** | Role selection, before provider invocation | Selects model capability and effort from generic criteria, then maps to CODEX, CLAUDE, or the explicit generic fallback configured in `leveling.yaml`. The resulting model and effort are shown on every role log line and recorded in telemetry (see `docs/configuration.md` § Role level label). The install wizard lets the operator choose manual (default; the host's model and effort are used as-is) or automatic (the policy picks per role by estimated load); only the mode is stored in `active.yaml`, and LEVELING data is read on demand (see `docs/configuration.md` § Manual or automatic level). |
