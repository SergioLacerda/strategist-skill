...
- **Provider fallback retirement**: the former `ask`/`native` provider substitution policy is retired. Active missions keep the selected Weapon immutable and fail closed with `role_invocation_failed`; old fallback records remain readable as historical evidence only. See `contracts/machine/provider-fallback.yaml`.
