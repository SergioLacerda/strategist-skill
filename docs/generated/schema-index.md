...
| `confidence-claim.schema.yaml` | Shared confidence envelope for claims emitted by Strategist agents. The percentage is an evidence-backed policy score; it is not an empirical probability unless calibration_status and an explicit ground-truth sample say so. |
...
| `initiative-advice.schema.yaml` | Consultative INITIATIVE advice. This schema is intentionally separate from LEVELING. No top-level model, provider, capability, effort, level, or level_source field may be written by this ability. |
| `initiative-result.schema.yaml` | Append-only INITIATIVE result correlated to one advice envelope. The result is advisory and gate-independent. |
