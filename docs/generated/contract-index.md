...
| Contract | Type | Origin | Description | Tests (best-effort) |
|---|---|---|---|---|
...
| `approval-gate` | gate | `.strategist/contracts/machine/approval-gate.yaml` | Approval gate between Archivist and Sniper. User reviews the refined analysis and decides to accept, request revision, or reject. Accepting triggers document... | — |
...
| `critical-hit.yaml` | — | `.strategist/contracts/machine/critical-hit.yaml` | **UNPARSEABLE**: mapping values are not allowed here in ".strategist/contracts/machine/critical-hit.yaml", line 137, column 96 | — |
| `intake` | agent_phase | `.strategist/contracts/machine/intake.yaml` | Classifies the user prompt via prompt-intake skill, generates mission_id, emits mission checkpoint, and routes quick-draw captures to the side-quest pipeline... | `tests/spec/specs/e2e-stop-conditions.feature`; `tests/spec/specs/forbidden-behaviors.feature`; `tests/spec/specs/token-economy.feature` |
...
| `opportunity-attack` | archivist_routine | `.strategist/contracts/machine/opportunity-attack.yaml` | ADR evaluation routine that runs INSIDE the Archivist, after all four refined artifacts are written. Evaluates whether the refined work warrants an ADR. Not ... | — |
| `preflight.yaml` | — | `.strategist/contracts/machine/preflight.yaml` | **UNPARSEABLE**: while parsing a block mapping in ".strategist/contracts/machine/preflight.yaml", line 29, column 7 expected <block end>, but found '<scalar>' in ".strategist/contracts/machine/preflight.yaml", line 31, column 27 | — |
| `quick-draw.yaml` | — | `.strategist/contracts/machine/quick-draw.yaml` | **UNPARSEABLE**: while parsing a block mapping in ".strategist/contracts/machine/quick-draw.yaml", line 14, column 7 expected <block end>, but found ',' in ".strategist/contracts/machine/quick-draw.yaml", line 14, column 28 | — |
| `scout-routing` | routing_decision | `.strategist/contracts/machine/scout-routing.yaml` | Machine contract for Scout, the internal pre-pipeline Intake Router. Scout classifies each request immediately after intake (and after quick_draw_detection f... | — |

Lifecycle: all contracts listed above are `active` unless superseded
by a newer ADR — see `docs/adr/README.md` for the ADR index.
