...
delegates each phase to a specialist, and blocks at an approval gate before anything
is written to your codebase.
...
Strategist adds a mandatory checkpoint between "planning" and "doing" — the **approval gate**.
No code is written until a human says so, no exceptions.
...
You → [Ranger]    → discovers requirements, maps context
      ...
      [Sniper]    → implements exactly what was approved
```
...
| Sniper | Executor | Implements the approved spec — surgically, minimally |
...
1. You see the full plan before a single file is touched
...
.analysis/
  refined/
    ...
    <mission_id>/
      ...
      tasks.md                  ← Archivist: numbered task list
...
a Sniper checks and sets it to `sniper_running` before touching any file, preventing
...
"I want to add feature X"
  ...
  🚦 Gate: review the plan, type "sim" to approve
        ↓
  Sniper executes tasks 1-by-1, updates mission_status
        ↓
  Done — artifacts in .analysis/done/
```
