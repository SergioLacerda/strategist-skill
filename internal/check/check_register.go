...
func init() {
	...
	checkCmd.Flags().BoolVar(&checkJSON, "json", false, "print one domain.PreflightResult JSON object ({schema_version, status, identity, bindings, warnings, next}) instead of the human-readable banner; never invokes providers or mutates state")
