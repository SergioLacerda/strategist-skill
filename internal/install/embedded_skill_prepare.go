...
func CheckEmbeddedDrift(opts PrepareEmbeddedOptions) (report PrepareEmbeddedReport, drift bool, err error) {
	...
	if writeErr := WriteCatalogAndMirrors(result, tmpRoot, tmpCatalogPath, tmpLockPath); writeErr != nil {
		return report, false, fmt.Errorf("check embedded drift: %w", writeErr)
	}
	...
	drift = catalogDrift || mirrorsDrift || lockDrift || len(result.Rejected) > 0
...
func mirrorsHaveDrifted(ingested []IngestedSkill, defaultsRoot, tmpRoot string) (bool, error) {
	for _, skill := range ingested {
		wantPath := filepath.Join(tmpRoot, "skills", skill.ID, "skill.yaml")
		want, err := os.ReadFile(wantPath) //nolint:gosec // G304: wantPath is generated by this same package into a freshly created temp dir
		if err != nil {
			return false, fmt.Errorf("check embedded drift: read %s: %w", wantPath, err)
		}
		gotPath := filepath.Join(defaultsRoot, "skills", skill.ID, "skill.yaml")
		got, err := os.ReadFile(gotPath) //nolint:gosec // G304: path built from caller-configured defaults root and this run's own ingested skill ids
