...
**Which layer this binds, by phase:** each pipeline Role owns its phase and
must invoke the one Weapon bound to its slot through the declared runtime
connector. Atomic and composite Weapons receive the same Role context;
composites may invoke multiple component Weapons, but cannot replace Role
ownership or write handoff state directly. A missing or uncallable Weapon is
`role_invocation_failed`; the parent agent's own prose is never a substitute.
