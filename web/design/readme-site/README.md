...
- **Sigil set (meaning is fixed):** ☉ Strategist · ⌖ Ranger · ❒ Archivist · ✜ Sniper · ✶ Wizard · ⛨ Approval Gate / portal (orange) · ⚲ Opportunist Attack · ⚑ Side Quest · ❖ Treasure Chest · ⚡ Quick Draw · ⧗ Initiative · ⛩ Dojo · ‡ / † Weapons (arsenal · individual) · ⌕ Discovery · ✎ Refinement · ◎ Execution (scope) · ✦ crest · ⟡ ornament.
...
- `ui_kits/console/` — **Strategist Console (v2)**, the current primary surface. A professional tabbed, bilingual (PT/EN, state-persisted) console — `index.html` + `i18n.js`:
  ...
  - **Skills** — special actions. From the repo: **Opportunist Attack** / **Side Quest**, **Treasure Chest**, **Quick Draw**. Plus two **product additions** (not yet in the repo): **Initiative** (assess the team before missions) and **Dojo** (controlled environment to test skills).
